...
const ages = inventors.map(inventor => inventor.passed - inventor.year).reduce((acc, curr) => acc + curr, 0);
//                                      OR
//                      Another solution without using .map
const totalAge = inventors.reduce((total, inventor) => total + (inventor.passed - inventor.year), 0);
console.log(ages, totalAge);
...
// const selectedNames = links.map(link => {
...
const sortedPeeps = people.sort((a, b) => {
...
const vehicleInstance = data.reduce((obj, vehicle) => {
    if (!obj[vehicle]) {
        obj[vehicle] = 0
    }
    obj[vehicle]++;
